(function() {
  ...
  window.Entry = (function() {
    ...
    Entry.prototype.generate_entry_xml = function() {
      return "<?xml version=\"1.0\" encoding=\"UTF-8\"?><entry xmlns=\"http://www.w3.org/2005/Atom\">" + ("<category label=\"Status\" scheme=\"http://masas.ca/categories/status\" term=\"" + (this.get('status').get('name')) + "\" />") + ("<category label=\"Severity\" scheme=\"http://masas.ca/categories/severity\" term=\"" + (this.get('severity').get('name')) + "\" />") + ("<category label=\"Certainty\" scheme=\"http://masas.ca/categories/certainty\" term=\"" + (this.get('certainty').get('name')) + "\" />") + ("<category label=\"Category\" scheme=\"http://masas.ca/categories/category\" term=\"" + (this.get('category').get('event_code')) + "\" />") + ("<category label=\"Icon\" scheme=\"http://masas.ca/categories/icon\" term=\"" + (this.get('subcategory').get('event_code')) + "\" />") + ("<title type=\"xhtml\"><div xmlns=\"http://www.w3.org/1999/xhtml\"><div xml:lang=\"en\">" + (this.get('title')) + "</div></div></title>") + ("<content type=\"xhtml\"><div xmlns=\"http://www.w3.org/1999/xhtml\"><div xml:lang=\"en\">" + (this.get('description')) + "</div></div></content>") + ("<point xmlns=\"http://www.georss.org/georss\">" + (this.get('location').get('latitude')) + " " + (this.get('location').get('longitude')) + "</point>") + ("<expires xmlns=\"http://purl.org/atompub/age/1.0\">" + (Date.today().add(7).days().toISOString()) + "</expires>") + "</entry>";
    };
    return Entry;
  })();
}).call(this);
